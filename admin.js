    ...
    const rows=(d.users||[]).map(x=>{
      ...
      const controls=x.role==='admin'
        ...
        : '<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+
          ...
          '<button class="btn btn-fire" onclick="saveAccess(\''+x.id+'\')">Salvar acesso</button>'+ '<button class="btn btn-ghost" onclick="trialUser(\''+x.id+'\')">+ Teste</button>'+ (s?.access_kind==='trial'?'<button class="btn btn-ghost" onclick="removeTrial(\''+x.id+'\')">- Teste</button><button class="btn btn-ghost" onclick="endTrial(\''+x.id+'\')">Encerrar teste</button>':'')+
...
window.removeTrial=async(id)=>{const raw=prompt('Quanto tempo deseja REMOVER do teste deste usuário? Ex.: 3');if(raw===null)return;const duration_value=Number(raw);if(!Number.isFinite(duration_value)||duration_value<=0){toast('Tempo inválido.');return}const unit=confirm('OK = DIAS\nCancelar = HORAS')?'days':'hours';const d=await api('admin_trial_user',{method:'POST',body:JSON.stringify({user_id:id,action:'remove',duration_value,duration_unit:unit})});if(d.ok){toast('Tempo removido do teste.');load('clientes')}else toast(d.error||'Erro ao alterar teste')};
