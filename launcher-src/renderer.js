...
function renderAccount(a,installedVersion){
  ...
  if(a.active){st.textContent='● ATIVA';st.className='status '+(a.warning?'warning':'active');$('days').textContent=(a.days_remaining??0)+' dias';$('expires').textContent='Vence em '+fmtDate(a.subscription?.current_period_end);renew.hidden=true;launch.disabled=false;if(a.warning){warn.hidden=false;warn.textContent='⚠ Sua assinatura termina em '+a.days_remaining+' dia'+(a.days_remaining===1?'':'s')+'. Renove para não interromper o acesso.'}else warn.hidden=true}
