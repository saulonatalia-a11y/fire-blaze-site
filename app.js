...
document.getElementById('register-btn').addEventListener('click',async()=>{
  ...
  const {data,error}=await sbClient.auth.signUp({email,password,options:{data:{name,whatsapp,whatsapp_opt_in:opt}}});
  if(error){msg.textContent=error.message;return;}
  if(data.session) await api('bootstrap',{method:'POST'});
  msg.textContent=data.session?'Conta criada. Próxima etapa: escolher o plano e pagar.':'Conta criada. Confira seu email para confirmar o cadastro.';
});
